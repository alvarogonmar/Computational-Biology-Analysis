---
title: "Activity_Basic_R_Exercises"
output:
  pdf_document: default
  html_document: default
---
```{r}
# 1
vec <- c(10, 11, 13, -1, 6, 3)
print(vec)
```

```{r}
# 2
media <- mean(vec) # Media
des_x <- sd(vec) # Desviacion estandar
var_x <- var(vec) # Varianza

est.x <- c(media, des_x, var_x)
print(est.x)
```

```{r}
#3
sequence <- 20:50
print(sequence)

mean_2 <- mean(20:60)
print(mean_2)

sum <- sum(51:91)
print(sum)
```

```{r}
# 4
random_v <- sample(-100:50, 10, replace = TRUE)
print(random_v)
```

```{r}
# 5
numbers <- numeric(10)
numbers[1] <- numbers[2] <- 1

for (i in 3:10) {
  numbers[i] <- numbers[i-1] + numbers[i-2]  # Fibonacci
}
print(numbers)
```

```{r}
# 6
vec_a <- c(10, 20, 30, 4, 50, -60)
# min and max vector a
max_a <- max(vec_a)
min_a <- min(vec_a)
min_max_a <- c(max_a, min_a)
print(min_max_a)
```

```{r}
# 7
multiplica <- function(vec1, vec2) {
  result <- vec1 * vec2
  return(result)
}
(multiplica(c(10, 20), c(3, 4)))
```


```{r}
# 8
counter <- function(vec, valor) {
  result <- sum(vec == valor)
  return(result)
}

print(counter(c(10, 20, 10, 7, 24, 7, 5), 7))
```
```{r}
# 9
enesimo <- function(vec, n) {
  return(vec[seq(n, length(vec), by = n)])
}

v <- 1:100
print(enesimo(v, 5))
```




